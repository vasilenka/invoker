...
export default class RangeCore extends React.Component {
  ...
  trackDelta = e => {
    ...
    let value;
    if (p.yAxis) value = this.getValue(e.clientY, s.mouseStart.y);
  ...
  setPercent = e => {
    if (!this.isSettingPercent) return;
    const rect = this.getTargetInfo().rect;

    let percent;
    if (this.props.yAxis)
      percent = ((e.clientY - rect.top) * 100) / rect.height;
    ...
    if (!this.props.disablePercentClamp) percent = this.clamp(0, 100, percent);
  ...
  handleMouseDown = e => {
    if (this.props.percent) this.startSetPercent(e);
    if (!this.state.isDragging) this.setState({ startIsDraggingOnMove: true });
  ...
  handleOnChange = (newValue, e) => {
    if (this.props.value !== newValue) this.props.onChange(newValue, e);
  ...
  handleMouseMove = e => {
    if (this.state.startIsDraggingOnMove) this.startIsDragging(e);
    if (this.props.percent) this.setPercent(e);
